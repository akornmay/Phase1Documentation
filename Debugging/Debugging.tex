...
The LED labelled \textbf{LED4} indicates the heart beat of the Module Management Controller (MMC).
...
The FC7 is equipped with six LEDs on the front panel.
 ...
 The naming convention of the LEDs can be found in figure \ref{pic:FC7front}.
 The purpose of the four standard LEDs can be seen seen in figure \ref{pic:LEDtable}.
 For the different flavours of FC7 cards in the Pixel DAQ system, the purpose of the user defined LEDs is summarised in table \ref{tab:UserLEDs}.
...
\subsection{FMC cards}
The two different FMC cards that make a FC7 board to either a FED or a FEC have a number of LED added to them.

\begin{description}
\item[FED FMC]\hfill\\
The FED FMC shown in figure \ref{pic:FEDFMC} is equipped with seven LEDs.
 None of them are currently used.
 All LEDs are \textbf{ON}.

\item[FEC FMC]\hfill\\
Tracker FEC and Pixel FEC use the same FMC card.
 Two LEDs on the card indicate the status of the +3.3V and the $\mathrm{V_{adj}}$ power supply, see figure \ref{pic:FECFMC}
\end{description}
...
\caption[]{\label{pic:FEDFMC}}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.45\textwidth}
\centering
\includegraphics[width=0.75\textwidth]{../images/FEC_FMC.png}
\caption[]{\label{pic:FECFMC}}
\end{subfigure}
\caption[FMC cards of the Pixel DAQ system]{\label{pic:PixelDAQFMC}
}
\end{figure}
